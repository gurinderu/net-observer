...
- **TriggerEngine** — rules ported from the oracle and grown since: `wedge`,
  ...
  `roam` (three identity axes — a BSSID hop, a Wi-Fi channel hop at the same
  DHCP router, or a new link address — classified by whether the link
  address was kept, and naming whether the DHCP lease was renewed; realm
  net-observer, node #126), `wifi-churn` (four or more Wi-Fi identity
  changes — bssid, link address, or channel — in a quarter hour read as one
  incident), `gw-mac-change` (pcap

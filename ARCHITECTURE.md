...
- **`announce`** — the second Event collector, and the passive tier's way of
  ...
  traffic and, during an operator-pressed scan, the sweep's ARP and the
  mDNS browse; never the periodic probes, which are ICMP, TCP and DNS and do
  not pass the filter). Our own frames are recognised by the
...
- **`singbox-log`** — what sing-box itself says. Under the passive tier the
  ...
  <duration>]? <component>[<tag>]?: <message>`) and classes it by the
  ...
  `default-iface-updated`, whose `node` column carries the interface name),
...
- `bin/net-observerd` wires everything and owns the DuckDB store; `bin/net-observer-cli`
  ...
  focus it observes unless its own actions menu holds the focus, its key state
...
`probing_edge`, `record_prune`), the records of an operator-pressed scan
(`neighbor_scan`, and `egress_scan` / `egress_dst`), the experiments and the
keyed entity tables. The default
...
| `connections(group_by)` | the newest `connection_sample` tick, grouped by `host` / `ip` / `ip-port` / `process` / `process-host` (`ConnectionsGroupBy`, in `types` like `HistoryWindow`) AND by `scope`: `ts_us, verdict, key, scope, count, upload, download, hosts`, ordered by `count DESC`, `hosts` the distinct names seen behind the key; a tick with no rows answers one row carrying only `ts_us` and `verdict`, so a `SKIP` is never an empty table. `process-host` groups by the pair (process, destination) instead of either alone, so a process's destinations are never folded together, and carries one extra column, `process`, next to `key` (realm net-observer, node #168). The whole tick travels, every scope: the readers fold it — the CLI's `connections` lists `external` only by default and ends with `+<n> internal (dns/plumbing), +<m> lan hidden — --all shows them` (`--all` lists everything with the `scope` column, `--scope internal\|lan\|external` one scope), the bar's window has the **external only** box — so one round trip serves both the table and the count of what was folded, and nothing was added to the request (realm net-observer, node #75) |
| `egress_scan` / `egress_dsts(scan_ts_us)` | the latest egress scan's header (`ts_us, iface, verdict, reason, duration_ms, packet_count, dst_count, byte_count`) and, pinned to that scan's own `ts_us`, its destinations (`dst_ip, packets, bytes`) loudest by bytes first — the two-read pin that keeps a scan the operator runs between the round trips from pairing one scan's header with another's destinations (the same race `air_scan`/`air_aps` close). The CLI's `egress` reads both, prints the destination table, and on a `SKIP` header prints the reason so an empty table is never read as "nothing leaves the interface" (realm net-observer, nodes #99, #170) |
...
- **Wire protocol** (`crates/net-observer-ipc`) — a request/response pair framed as
  ...
  - `Request::Control(ControlCmd)` → `Response::Control(ControlResult)` — the
    ...
    non-read request. Nine commands today — `ControlCmd::KickstartProxy`,
    `SetObserving(bool)`, `SetProbing(ProbingTier)`, `FreezePcap`,
    `ScanNeighbors(ScanOptions)`, `ScanAir`, `ScanTopology`, `ScanEgress`,
    `StartExperiment { minutes }` — every one behind the same peer-credential
    check and none behind a config switch. `ScanEgress` captures the physical
    uplink's outgoing packets for ~5 s and folds them by destination, read back
    with `egress` (realm net-observer, node #170).

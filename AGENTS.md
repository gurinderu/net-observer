...
| The nix packages build (daemon, CLI, bar) — from a fresh `Cargo.nix` | the derivations `nix build .#net-observerd .#net-observer-cli .#net-observer-bar` produce | CI job `nix-build` on macos-latest (`gh pr checks <n>`); locally only on a Mac with nix — from Linux the darwin attributes evaluate (`nix eval --raw .#packages.aarch64-darwin.net-observerd.drvPath`) but cannot be built | agent (via CI) |
...
- **CI**: three workflows on `macos-latest`, all gating pull requests *and* pushes to `main` — `lints` (`cargo fmt --all --check`, clippy with `-D warnings`, `RUSTFLAGS: -D warnings`, plus a dedicated `cargo clippy -p net-observer-bar` step), `tests` (`cargo build --all`, `cargo test --all`, plus a dedicated `cargo build -p net-observer-bar` step — the macos-latest runner carries the Metal Toolchain, confirmed 2026-09-01), and `nix-build` (first `crate2nix generate` inside `nix develop` diffed against the checked-in `Cargo.nix`, then `nix build` of the three packages on macos-latest, so a stale `Cargo.nix` and a broken crate2nix packaging are both caught before merge). There are no post-merge-only jobs, so PR↔main parity is complete.

...
- **The daemon's live behavior** (it really writes that row, the ring really freezes, the passive tier really silences the wire). Needs a root run on this Mac against a real network, and the owner has not named how he wants that observed. Until he does, such a claim is closed only by an observation named aloud *before* looking — never by re-reading the diff.

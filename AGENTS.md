...
- **The daemon's live behavior where neither the socket nor the record shows it** (the ring really freezes, the passive tier really silences the wire, the bar shows what the daemon said). What the daemon *reports* is reachable over the socket and what it *wrote* through an offline copy of the record (the two rows above, both first used 2026-09-17); the pcap ring and freeze copies are group-readable like the record; what stays unreachable is the bar's window. That needs a run on this Mac against a real network, and the owner has not named how he wants that observed. Until he does, such a claim is closed only by an observation named aloud *before* looking — never by re-reading the diff.

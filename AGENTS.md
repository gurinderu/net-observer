...
| The nix packages build (daemon, CLI, bar) — crate2nix generation included | the derivations `nix build .#net-observerd .#net-observer-cli .#net-observer-bar` produce | CI job `nix-build` on macos-latest (`gh pr checks <n>`); locally only on a Mac with nix — a Linux box cannot even evaluate the darwin attributes (IFD builds the generator for the target system) | agent (via CI) |
...
- **Gotchas**:
  ...
  - crate2nix generates `Cargo.nix` at evaluation time from `Cargo.lock` (no generated file in the repo): evaluation itself needs network and import-from-derivation. From Linux only `packages.x86_64-linux.*` attributes evaluate — the darwin ones would build the generator for darwin. Native-crate overrides live in `crateOverrides` in `flake.nix`, one comment per override (realm net-observer, node #80).

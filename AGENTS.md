...
| The nix packages build (daemon, CLI, bar) — from a fresh `Cargo.nix` | the derivations `nix build .#net-observerd`, `.#net-observer-cli`, `.#net-observer-bar` produce | the `nix-build` workflow on macos-latest, one job per package (`nix-build-daemon`, `nix-build-cli`, `nix-build-bar`; `gh pr checks <n>`) — first green run observed 2026-09-16 on the pull request that introduced the route (cold: daemon ~30 min, CLI ~23 min; the bar needed the video-crate override first and then ran cold in ~21 min on a later pull request), as the row below dates its Linux observation; locally only on a Mac with nix — from Linux the darwin attributes evaluate (`nix eval --raw .#packages.aarch64-darwin.net-observerd.drvPath`) but cannot be built | agent (via CI) |
| The crate2nix route builds DuckDB from source (`buildRustCrate` driving `libduckdb-sys`'s build script) | the `store` and `triggers` crate derivations from the same `Cargo.nix` | `nix build .#checks.<system>.store-crate --print-build-logs`, then `.#checks.<system>.triggers-crate`, each exit code read on its own — observed on x86_64-linux (2026-09-16, ~25 minutes cold on 22 cores); the aarch64-darwin attributes exist but have not been run | agent |
| The checked-in `Cargo.nix` is fresh | the regenerated file | inside `nix develop`: `crate2nix generate`, then `git diff --exit-code Cargo.nix crate-hashes.json` — the same pair of commands CI runs | agent |
| Pure logic inside an Apple-only crate (the bar's table folds and captions, `macos` parsers and the capture-start seam) — on a Linux box | the same functions compiled in a scratch crate OUTSIDE the repo against the real workspace crates (`types`, `net-observer-ipc`, `collector-*`) | copy the gpui-free / objc-free functions and their tests into a throwaway crate in the scratchpad, point its `[dependencies]` at the workspace crates by path, `cargo test` and `cargo clippy -- -D warnings` there, each exit code read on its own — first used 2026-09-17, caught a borrow error, an `enum_variant_names` lint and a type mismatch before CI; the gpui/objc half stays a CI claim | agent |
| The daemon's live behavior, read over the socket (what it reports: status, incidents, the named diagnoses) | the running daemon's control socket on the owner's Mac — world-rw, so no root needed | `ssh <mac> "printf '\"Status\"\n' \| nc -U -w 5 /var/lib/observer/observer.sock"` — newline-delimited JSON, one request per line: `"Status"`, `{"Incidents":{"limit":N}}`, `{"Query":"Silences"}`, `{"Query":{"Why":{"ts_us":…}}}`, `{"Query":{"Connections":{"group_by":"process"}}}`; first used 2026-09-17. This narrows the Ceiling bullet below: the daemon's live behavior stays unreachable only where the socket does not expose it (root-only files, the bar) | agent |
| The record's contents (rows really written) | a copy of the Mac's `observer.duckdb` + `.wal` (both 0644 since the owner opened them), read offline on Linux | `scp` both files, `cargo build -p net-observer-cli`, then `target/debug/net-observer-cli --db <copy>/observer.duckdb query "<SQL>"` — the first open replays the WAL and takes minutes; every later query is fast; first used 2026-09-17 | agent |
...
- **The daemon's live behavior where neither the socket nor the record shows it** (the ring really freezes, the passive tier really silences the wire, the bar shows what the daemon said). What the daemon *reports* is reachable over the socket and what it *wrote* through an offline copy of the record (the two rows above, both first used 2026-09-17); what stays out of reach is the root-only files (the pcap ring and what a freeze copies out of it) and the bar's window. Those need a root run on this Mac against a real network, and the owner has not named how he wants that observed. Until he does, such a claim is closed only by an observation named aloud *before* looking — never by re-reading the diff.
- **A change rebuilt only the affected crates.** Reachable only by reading a `nix build --print-build-logs` run after a real change on a warm store (the owner's Mac, or a second CI run with a hit cache): the list of derivations that build is the observation; a green build alone says nothing about reuse.
...
crates/
  ...
  collector-{link,proxy,dns,route,host,wifi,neighbors,air,connections,announce}/   # one collector per crate
...
- **Maximize Rust.** Prefer a pure-Rust crate for every component. Native (C/C++) deps are allowed only where no adequate equivalent exists, and each exception is named here. Current exceptions: **DuckDB** (no pure-Rust DB offers native `ASOF JOIN`) and, for v1 only, **`tcpdump` as a child process** — three times over: the pcap ring, the short-lived LLDP/CDP capture of the topology patrol, and the announce listener's streaming capture (`-w -` to a pipe). The child is only ever the *capture*; every frame it delivers is decoded in-process by pure Rust (`lldp`, `collector-announce` with `etherparse` and `simple-dns`), never by `tcpdump`'s text output (realm net-observer, node #92). Any future GUI is `gpui`.
- **v1 = observe + detect, never act.** No `launchctl kickstart`, no watchdog, no notifications. Triggers fire *passive* handlers only (record an incident, freeze the pcap ring). Acting handlers sit behind the same `Condition → Handler` interface but are not in v1. That is about what the daemon does by itself: a control command the operator sends by hand (a neighbour scan rung, a proxy kickstart) is never gated by config — the invocation is the sanction; only the peer-uid check applies (realm net-observer, node #91).
- **SKIP, never silence.** A probe that cannot run emits a `SKIP` verdict rather than going quiet — absence of a signal is itself diagnostic. The passive probing tier is not an exception to this: it withholds every probe, but the affected collectors keep emitting one sample per tick with the withheld fields `SKIP`, and the triggers read that as no measurement — never as a healthy gateway, never as a drop. The one sanctioned exception is an operator pause (`ControlCmd::SetObserving`): a paused daemon stops collecting outright instead of emitting per-tick synthetic `SKIP` samples. That silence is *bracketed*: each pause/resume edge writes a durable `observing_edge` row through the `Store` and publishes a `StreamFrame::Observing` on the realtime bus, so the gap is bounded and attributable (to a `ts_us` and a control-socket `peer_uid`). Unbracketed silence is a bug. The probing tier (`ControlCmd::SetProbing`, `passive` by default) is the second sanctioned withholding — every withheld probe still lands as `SKIP` and every tier switch is a durable `probing_edge` `(realm net-observer, node #88)`, with the same one exception the pause has: a tick in flight across the switch is dropped whole at the source (never written, never published), bracketed by that `probing_edge` row and an info log. Samples the bounded post-resume drain keeps out of the trigger window are not a second exception: each is still written to DuckDB and published on the bus, and the drain is bounded by a monotonic deadline and a drop cap and reports its totals. The observing state is process-scoped and deliberately never persisted — a restart always resumes collecting.
...
- **Gotchas**:
  ...
  - Building the bar needs no Metal toolchain at build time: the workspace pins gpui's `runtime_shaders` feature (see `Cargo.toml`), so the shaders compile at app launch by the system Metal framework instead of by `xcrun -sdk macosx metal` in the build script — the reason the bar was once unbuildable without a full Xcode and unpackageable by nix. What its build script still runs is bindgen over `dispatch.h`, which needs libclang and the SDK headers: the nix package takes both from `bindgenHook` (the gpui override in `flake.nix`); a cargo build on the Mac takes libclang from wherever `xcode-select` points — the Command Line Tools suffice — and the dev shell adds nothing for it. **Build the bar from inside `nix develop`** all the same, so the gate runs with the pinned toolchain; a bindgen failure there is a libclang-lookup problem, not a Metal one.
  ...
  - `Cargo.nix` is CHECKED IN, generated from `Cargo.lock` by the crate2nix CLI the dev shell carries — the eval-time generator cannot read the duckdb-rs fork's workspace inheritance (realm net-observer, node #95). Whenever `Cargo.lock` changes, run `crate2nix generate` inside `nix develop` and commit `Cargo.nix` and `crate-hashes.json`; the `nix-build-daemon` CI job regenerates and fails on a diff, so a stale file does not survive a pull request. Evaluation is pure (no network, no import-from-derivation): from Linux the darwin attributes evaluate, they just cannot be built there. Native-crate overrides live in `crateOverrides` in `flake.nix`, one comment per override (realm net-observer, node #80).
  - The schema is additive **forward only**: `SCHEMA_SQL` adds columns on open and never removes one, so a record a newer build has opened keeps its wider tables, and an older build run against it afterwards has every positional `INSERT` into those tables refused by the binder — observed on the owner's Mac 2026-09-17 between two `darwin-rebuild switch`es (`table host_sample has 7 columns but 4 values were supplied`, `table link_sample has 20 columns but 14 values were supplied`), one gap line per dropped sample for the whole stretch the older build ran (realm net-observer, node #150). Never switch the Mac back to an older revision without expecting that gap. The daemon now names it once at startup — `schema drift: <table> has <actual> columns, this build writes <expected> …`, WARN, one line per widened table, and it keeps running for the tables it does fit — so the first line of the log after a switch says whether the record is being written at all.
...
- **CI**: three workflows on `macos-latest`, all gating pull requests *and* pushes to `main` — `lints` (`cargo fmt --all --check`, clippy with `-D warnings`, `RUSTFLAGS: -D warnings`, plus a dedicated `cargo clippy -p net-observer-bar` step), `tests` (`cargo build --all`, `cargo test --all`, plus a dedicated `cargo build -p net-observer-bar` step — the macos-latest runner carries the Metal Toolchain, confirmed 2026-09-01), and `nix-build` (three jobs on macos-latest, one per package; each saves only on `main`, purging its older keys, so pull requests restore `main`'s store and rebuild only what changed — the Actions cache cap is 10 GB, and the `rust-cache` steps of `lints` and `tests` save only on `main` for the same reason: `nix-build-daemon` first diffs `crate2nix generate` inside `nix develop` against the checked-in `Cargo.nix`, then builds `.#net-observerd`; `nix-build-cli` and `nix-build-bar` build the other two — so a stale `Cargo.nix` and a broken crate2nix packaging are both caught before merge. First green run 2026-09-16). Each nix job also pulls from and, on `main`, pushes to the Cachix binary cache `net-observer` (`cachix/cachix-action`; the token lives in the repository secrets, and a fork or a repository without it pulls only), which is what the owner's Mac substitutes from instead of building (realm net-observer, node #133). There are no post-merge-only jobs, so PR↔main parity is complete.

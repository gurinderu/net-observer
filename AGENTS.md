...
- **CI**: two workflows on `macos-latest`, both gating pull requests *and* pushes to `main` — `lints` (`cargo fmt --all --check`, clippy with `-D warnings`, `RUSTFLAGS: -D warnings`, plus a dedicated `cargo clippy -p net-observer-bar` step) and `tests` (`cargo build --all`, `cargo test --all`, plus a dedicated `cargo build -p net-observer-bar` step — the macos-latest runner carries the Metal Toolchain, confirmed 2026-09-01). There are no post-merge-only jobs, so PR↔main parity is complete.
